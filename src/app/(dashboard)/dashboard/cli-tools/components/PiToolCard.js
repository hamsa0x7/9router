...
export default function PiToolCard({
  ...
  useEffect(() => {
    if (piStatus?.installed && !hasInitializedModel.current) {
      hasInitializedModel.current = true;
      const provider = piStatus.config?.providers?.["9router"];
      if (provider?.models?.[0]?.id) setSelectedModel(provider.models[0].id);
    }
  }, [piStatus]);
  ...
  const getManualConfigs = () => {
          ...
          models: [
            {
              ...
              name: `${selectedModel || "model"} via 9Router`,
              reasoning: true,
              input: ["text", "image"],
    ...
    return [
      { filename: "~/.pi/agent/models.json", content: jsonContent },
    ];
  };
              ...
              <div className="flex flex-col gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className="font-medium text-yellow-600 dark:text-yellow-400">Pi Agent not detected locally</p>
                    <p className="text-sm text-text-muted mt-1">Install via npm:</p>
                    <code className="block mt-2 p-2 bg-black/20 rounded text-xs font-mono">npm install -g @earendil-works/pi-coding-agent</code>
          ...
          {!checking && piStatus?.installed && (
            <>
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleReset} disabled={!piStatus?.has9Router} loading={restoring}>
