...
const getBackupPath = () => path.join(getLocalBackendDir(), "providers", "auth.json.9router-backup");
...
// ── Check if a base_url points to 9Router ──────────────────────────────
const is9RouterUrl = (baseUrl) => {
  if (!baseUrl) return false;
  return baseUrl.includes(":20128") || baseUrl.includes(":3000") || baseUrl.includes("9router");
};

// ── Check if 9Router is configured ─────────────────────────────────────
const has9RouterConfig = (authFile) => {
  if (!authFile?.providers) return false;
  const provider = authFile.providers[PROVIDER_NAME];
  if (!provider) return false;
  return is9RouterUrl(provider.base_url);
};
...
export async function GET() {
  try {
    ...
    // Detect if lmstudio is already configured for a non-9Router endpoint
    let lmstudioConflict = false;
    if (provider && !is9RouterUrl(provider.base_url)) {
    ...
    return NextResponse.json({
      installed: true,
      config: authFile,
      has9Router: has9RouterConfig(authFile),
      lmstudioConflict,
...
// ── POST - Apply 9Router as LM Studio provider + switch to local mode ──
export async function POST(request) {
  try {
    ...
    if (existingProvider && !is9RouterUrl(existingProvider.base_url) && !overwrite) {
    ...
    if (existingProvider && !is9RouterUrl(existingProvider.base_url)) {
    ...
    // Clean up legacy lc-9router provider if present
    if (authFile.providers?.["lc-9router"]) {
      delete authFile.providers["lc-9router"];
    }
    ...
    return NextResponse.json({
      success: true,
      message: "Settings applied. Restart Letta CLI, then use /model to select a 9Router model.",
      needsRestart: true,
    });
  } catch (error) {
...
// ── DELETE - Remove 9Router configuration ──────────────────────────────
export async function DELETE() {
  try {
    ...
    const message = restored
      ? "9Router config removed. Your original LM Studio provider has been restored. Restart Letta CLI to take effect."
      : "9Router config removed. Restart Letta CLI to take effect.";
