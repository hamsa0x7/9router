...
function parseArgs() {
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === "--llama-port" || args[i] === "-p") {
      config.llamaPort = parseInt(args[i + 1], 10) || DEFAULT_LLAMA_PORT;
      i++;
...
Usage: node llama-cpp-server.js [options]

Options:
  --llama-port, -p <port>   Port for llama.cpp compatible API (default: ${DEFAULT_LLAMA_PORT})
...
function createProxyServer(config) {
  ...
  return http.createServer(async (req, res) => {
    const url = new URL(req.url, `http://localhost:${config.llamaPort}`);
    const fs = require("fs");
...
async function main() {
  ...
  server.listen(config.llamaPort, "0.0.0.0", () => {
    console.log(`[llama-cpp] Server running at http://0.0.0.0:${config.llamaPort}/v1`);
    console.log(`[llama-cpp] Proxying to http://${config.mainHost}:${config.mainPort}/api/v1`);
    console.log(`[llama-cpp] Tools can now connect to 9router as llama.cpp backend`);
  });

  server.on("error", (err) => {
    if (err.code === "EADDRINUSE") {
      console.error(`[llama-cpp] Port ${config.llamaPort} already in use`);
    } else {
      console.error(`[llama-cpp] Server error: ${err.message}`);
    }
    process.exit(1);
  });
